...
def parse_args():
    ...
    parser.add_argument('--N',
                        default=5,
                        type=int,
                        help='times of sampling while computing causal loss')
    ...
    parser.add_argument('--resume',
                        action="store_true",
                        help="resume task model or not")
    parser.add_argument('--random_sparsity',
                        action="store_true",
...
def main(args):
    ...
    if args.task:
        ...
        for data in train_set:
            data.y[:] = task_model(
                data.x.to(device),
                data.edge_index.to(device)).argmax(dim=1).cpu().detach()
